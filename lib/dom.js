...
const evalScripts = (source, root = null) => {
  ...
  scripts.length > 0 && scripts.forEach(t => {
    ...
    try {
      root ? root.insertBefore(s, root.children[0]) : t.parentNode.replaceChild(s, t)
    } catch(e){
      errors.push(e)
...
export default (root, duration, events) => (markup, cb) => {
  ...
  const oldMetas = [...document.head.querySelectorAll('meta')]
  const newMetas = [...dom.head.querySelectorAll('meta')]
  ...
  const render = tarry(
    () => {
      ...
      oldMetas.forEach(meta => document.head.removeChild(meta))
      newMetas.forEach(meta => document.head.appendChild(meta))
      cb(title, main)
      evalScripts(main)
